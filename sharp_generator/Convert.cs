...
public class Convert
{
	...
	void BuiltinClass(Api.BuiltinClass c, string dir, bool hasPointer)
	{
		...
		foreach (var member in membersWithFunctions)
		{
			file.WriteLine($"\t\t{member}_getter = GDExtensionMain.extensionInterface.variant_get_ptr_getter((Native.GDExtensionVariantType)Variant.Type.{Fixer.Type(c.name)}, new StringName(\"{member}\")._internal_pointer);");
			file.WriteLine($"\t\t{member}_setter = GDExtensionMain.extensionInterface.variant_get_ptr_setter((Native.GDExtensionVariantType)Variant.Type.{Fixer.Type(c.name)}, new StringName(\"{member}\")._internal_pointer);");
		}
		for (var i = 0; i < constructorRegistrations.Count; i++)
		{
		...
		if (hasPointer)
		{
			file.WriteLine($"\t\t__destructor = GDExtensionMain.extensionInterface.variant_get_ptr_destructor((Native.GDExtensionVariantType)Variant.Type.{Fixer.Type(c.name)});");
		}
		if (c.constants != null)
		{
			foreach (var con in c.constants)
	...
	void Constructor(Api.BuiltinClass c, Api.Constructor constructor, StreamWriter file, List<string> constructorRegistrations, Documentation.Constructor doc, bool hasPointer)
	{
		...
		constructorRegistrations.Add($"\t\t{m} = GDExtensionMain.extensionInterface.variant_get_ptr_constructor((Native.GDExtensionVariantType)Variant.Type.{Fixer.Type(c.name)}, {constructor.index});");
	...
	static void Enum(Api.Enum e, StreamWriter file, Documentation.Constant[] constants = null)
	{
	...
	static string FixDefaultValue(string value, string type)
	{
	...
	static void EqualAndHash(string className, StreamWriter file)
	{
	...
	void Class(Api.Class c, string dir)
	{
		...
		if (c.properties != null)
		{
			foreach (var prop in c.properties)
			{
				...
				file.Write($"\tpublic {Fixer.Type(type)} {Fixer.Name(Fixer.SnakeToPascal(prop.name)) + (hasEnumOfSameName ? "Value" : "")} {{ ");
